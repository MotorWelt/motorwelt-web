...
const Card: React.FC<{ className?: string; children: React.ReactNode }> = ({
  ...
  <div
    className={`flex h-full flex-col rounded-2xl border border-white/[0.06] bg-mw-surface/72 backdrop-blur-md transition hover:border-white/12 ${className}`}
  >
    {children}
  </div>
);
...
export default function HomePage({
  ...
  useEffect(() => {
    const updateDesktopState = () => setIsDesktop(window.innerWidth >= 768);
    updateDesktopState();
  ...
  const editControlsVisible = canEditHome && !spectatorMode && isDesktop;
    ...
    return (
      <section className="relative z-20 -mt-10 pb-4 sm:-mt-12 sm:pb-6 lg:-mt-14">
        <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 xl:px-10 2xl:max-w-[1560px]">
          <div className="rounded-[28px] border border-white/[0.08] bg-black/35 p-3 shadow-[0_18px_60px_rgba(0,0,0,.35)] backdrop-blur-xl sm:p-4">
  ...
  function renderEmptySectionNotice(title: string, message: string) {
    return (
      <div className="rounded-2xl border border-dashed border-white/10 bg-mw-surface/35 p-8 text-center backdrop-blur-md">
        <div className="inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-black/20 px-3 py-1 text-[10px] uppercase tracking-[0.22em] text-gray-400">
          <span className="h-2 w-2 rounded-full bg-[#0CE0B2]" />
          Próximamente
        </div>
